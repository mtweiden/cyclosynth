...
Cost weights match the synthesiser's `gates_cost`: T = 1, Q = 3.5.
H, S, X, Y, Z are free.
"""
...
OUT_PATH = "comparison_sqrtt_slope.png"

# Synthesizer's optimisation weights — must match `gates_cost` in
...
SUB_OFFSET = 0.15
# Stride between ε bucket centres. <1 packs the buckets closer.
X_STRIDE = 0.6
# Global font scale.
FS_TICK = 14
FS_AXIS = 16
...
def main():
    ...
    # Colour for the Clifford+T (Q=0) endpoint scatter.
    ...
    # Drawn in light grey behind the per-bucket coloured segments so
    ...
    for j, eps in enumerate(epsilons):
        ...
        for q_c, t_c in plotted:
            color = color_Q if q_c < t_c else "lightgray"
        ...
        if plotted:
            ...
            ax.plot([x_q, x_t], [mean_q, mean_t], "-",
                    color="white", linewidth=2.2, alpha=0.95, zorder=4)
            ax.scatter([x_q, x_t], [mean_q, mean_t], s=42, color="white",
    ...
    legend_handles = [
        ...
        plt.Line2D([0], [0], color="white", linewidth=2.2,
