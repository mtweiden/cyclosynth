...
//! pre-allocated `LenstraScratch` (allocated once via rayon's `map_init`), this
...
// ─── LenstraScratch: per-thread pre-allocated working buffers ────────────────
...
pub fn build_q(scratch: &mut LenstraScratch, y: &[Float; 8], k: u32, eps: Float) {
...
pub fn build_center(scratch: &mut LenstraScratch, y: &[Float; 8], _k: u32, eps: Float) {
    let prec = scratch.prec;
    scratch.eps_rf.assign(rfv(prec, eps));
...
fn compute_qgram_inplace(scratch: &mut LenstraScratch) {
    let prec = scratch.prec;
...
fn gs_qgram_inplace(scratch: &mut LenstraScratch) {
    let prec = scratch.prec;
...
pub fn lll_qgram_8(scratch: &mut LenstraScratch) {
    scratch.reset_basis();
...
pub fn cholesky_8(scratch: &mut LenstraScratch) -> bool {
    let prec = scratch.prec;
    // Zero L
    for i in 0..8 {
        for j in 0..8 {
...
fn lu_solve_inplace(scratch: &mut LenstraScratch) -> bool {
    let prec = scratch.prec;
...
mod tests {
    ...
    fn lll_returns_unimodular_at_eps_1e_4() {
        ...
        let mut scratch = LenstraScratch::new(prec);
