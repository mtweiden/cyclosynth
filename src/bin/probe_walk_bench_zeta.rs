...
use cyclosynth::synthesis::lattice::zeta::{find_aligned_lattice_points_with_stop, IntScratch16};
use cyclosynth::synthesis::lattice::zeta::brute::uv_to_lattice_y_zeta;
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("usage: probe_walk_bench_zeta <theta> <eps> <k> [<parity: 0|1>] [<bound_sq>]");
        std::process::exit(2);
    }
    ...
    println!(
        "probe_walk_bench_zeta: rz({theta}) eps={eps:e} k={k} threads={}",
