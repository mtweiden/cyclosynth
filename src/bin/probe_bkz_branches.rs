...
fn main() {
    ...
    println!(
        "=== BKZ branch frequency: theta={} eps={:e} ===",
        theta, eps
    );
    match r {
        Some(r) => println!(
    ...
    if total > 0 {
        ...
        println!(
            "    branch 1 (single ±1):          {b1:>10} ({:>5.1}%)",
            pct(b1)
        );
        println!(
            "    branch 2 (some ±1, pivot):     {b2:>10} ({:>5.1}%)",
            pct(b2)
        );
        println!(
            "    branch 3 success (gcd=1):      {b3:>10} ({:>5.1}%)",
            pct(b3)
        );
        println!(
            "    branch 3 non-primitive (gcd>1): {b3np:>9} ({:>5.1}%)",
            pct(b3np)
        );
    }
}
