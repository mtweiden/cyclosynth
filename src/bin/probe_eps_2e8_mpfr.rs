//! Verify synthesize_v_mpfr is correct: at ε=2e-8 it should find at lde=23
//! (matches f64 path).
...
fn main() {
    ...
    eprintln!("─── Q at ε=2e-8 via synthesize_v_mpfr (sanity-check) ───");
    let synth = SynthesizerQ::new(eps);
    eprintln!("  config: max_lde={}, min_lde={}", synth.max_lde, synth.min_lde);
    let t = Instant::now();
