...
mod probes;
    ...
    /// `gates_cost` charges per syllable (split on off-diagonal
    /// H/X/Y) by the net Q-power class, not per gate. `q_cost_x2 = 6` ⇒
    /// √T-class syllable = 6 half-units (3 T), T-class = 2 (1 T), Clifford = 0.
    #[test]
    fn gates_cost_syllable_model() {
        // Single gates.
        ...
        // QQ = T (T-class), TT = S (Clifford) — composed within a syllable.
        assert_eq!(gates_cost("QQ", 6), 2);
        assert_eq!(gates_cost("TT", 6), 0);
        // H/X/Y break syllables, so each √T is then charged on its own.
        ...
        // S, Z add multiples of 4, so never change a syllable's class.
        assert_eq!(gates_cost("QSZ", 6), 6); // √T-class
        assert_eq!(gates_cost("TS", 6), 2); // T-class
        // q_cost_x2 reweights only the √T-class syllables.
