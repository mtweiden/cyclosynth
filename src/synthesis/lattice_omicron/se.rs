...
/// Evaluate the n=6 bilinear form `B(x) = a₀a₁ + a₁a₂ + a₂a₃ + b₀b₁ + b₁b₂ + b₂b₃`
/// where `x = (a₀,a₁,a₂,a₃, b₀,b₁,b₂,b₃)` in the Z[ξ] cyclotomic basis.
///
/// This is the √3-part of |u|² + |t|² that must vanish for SU(2) unitarity
/// (consecutive-coord pairs only, all + signs — differs from n=4 which has
/// − signs at the (0,3) and (4,7) cross-terms).
///
...
pub fn bilinear_b(x: &[i64; 8]) -> i128 {
    let (a0, a1, a2, a3) = (x[0] as i128, x[1] as i128, x[2] as i128, x[3] as i128);
    let (b0, b1, b2, b3) = (x[4] as i128, x[5] as i128, x[6] as i128, x[7] as i128);
